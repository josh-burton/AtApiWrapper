...
public class VehicleLocationListFragment extends ListFragment {
    ...
    public void onResume() {
        ...
        realtimeService.vehicleLocations().enqueue(new Callback<ServerResponse<VehicleLocationResponse>>() {
            @Override
            public void onResponse(Call<ServerResponse<VehicleLocationResponse>> call, Response<ServerResponse<VehicleLocationResponse>> response) {
                if (response.isSuccessful()) {
                    VehicleLocationResponse serverResponse = response.body().getResponse();
                    if (serverResponse.getVehicleLocations().size() > 0) {
                        mVehicleLocations.clear();
                        mVehicleLocations.addAll(serverResponse.getVehicleLocations());
            ...
            @Override
            public void onFailure(Call<ServerResponse<VehicleLocationResponse>> call, Throwable t) {
                t.printStackTrace();
                Toast.makeText(getActivity(), "Error", Toast.LENGTH_LONG).show();
            }
        });
